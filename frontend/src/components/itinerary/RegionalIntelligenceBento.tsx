...
export function RegionalIntelligenceBento({ data, cityName, stateName }: Props) {
  if (!data) return null;
  ...
  return (
    ...
    >
      {/* Editorial Subheader */}
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 border-b border-[#E8DEC8] pb-5">
        <div>
          <span className="font-heading text-xs font-extrabold uppercase tracking-widest text-[#C85A32] bg-[#C85A32]/10 border border-[#C85A32]/20 px-3 py-1 rounded-full inline-block">
            Field Intelligence &amp; Regional Dossier
          </span>
          <h3 className="font-heading text-2xl sm:text-3xl font-extrabold text-[#12213B] tracking-tight mt-2">
            Essential {resolvedTitle} Briefing
          </h3>
        </div>
        <span className="font-sans text-xs text-neutral-500 font-medium">
          Dynamically synthesized across verified regional registries
        </span>
      </div>

      {/* 2x2 Bento Grid with Ambient Radial Halos */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
        {/* CARD 1: GASTRONOMY & LOCAL HEARTHS */}
        <motion.div
          variants={cardItemVariants}
          whileHover={{ y: -5, scale: 1.01, transition: { duration: 0.25 } }}
          className="group relative rounded-3xl p-7 bg-white/80 backdrop-blur-xl border-2 border-[#E8DEC8] hover:border-[#C85A32] shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden flex flex-col justify-between"
        >
          ...
          <div className="relative z-10 space-y-5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-heading text-xs uppercase font-extrabold tracking-widest text-[#C85A32]">
                    Gastronomy Dossier
                  </span>
                  <h4 className="font-heading text-2xl font-extrabold text-[#12213B] tracking-tight">
                    Food Recommendations
                  </h4>
                </div>
              </div>
              <span className="px-3 py-1 rounded-full bg-amber-100 text-[#C85A32] font-heading text-xs font-bold uppercase tracking-wider">
                Local Hearth
              </span>
            </div>
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(item.dishes || []).map((dish, dIdx) => (
                      <span
                        key={dIdx}
                        className="font-sans text-xs sm:text-sm font-semibold px-3 py-1.5 rounded-xl bg-white border border-[#E8DEC8] text-neutral-800 shadow-2xs hover:bg-[#FAF7F2] transition-colors"
                      >
                        {dish}
                      </span>
                    ))}
                  ...
                  {(item.contextNote || item.notes) && (
                    <p className="font-sans text-xs text-neutral-500 italic pt-0.5">
                      {item.contextNote || item.notes}
                    </p>
                  )}
                </motion.div>
        ...
        {/* CARD 2: TACTILE FIELD ADVISORIES */}
        <motion.div
          variants={cardItemVariants}
          whileHover={{ y: -5, scale: 1.01, transition: { duration: 0.25 } }}
          className="group relative rounded-3xl p-7 bg-white/80 backdrop-blur-xl border-2 border-[#E8DEC8] hover:border-indigo-500 shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden flex flex-col justify-between"
        >
          ...
          <div className="relative z-10 space-y-5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-heading text-xs uppercase font-extrabold tracking-widest text-indigo-600">
                    On-Ground Intuition
                  </span>
                  <h4 className="font-heading text-2xl font-extrabold text-[#12213B] tracking-tight">
              ...
              <span className="px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 font-heading text-xs font-bold uppercase tracking-wider">
                Insider Rules
              </span>
            </div>
            ...
            <div className="space-y-3 pt-1">
              {(data.travelTips || []).map((tip, idx) => (
                ...
                >
                  <div className="w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-mono font-bold text-xs shrink-0 mt-0.5">
                    0{idx + 1}
                  </div>
                  <p className="font-sans text-sm sm:text-[15px] text-neutral-800 leading-relaxed font-medium">
                    {tip}
                  </p>
                </motion.div>
              ))}
        ...
        {/* CARD 3: BEST TIME TO VISIT & SOLAR RHYTHMS */}
        <motion.div
          variants={cardItemVariants}
          whileHover={{ y: -5, scale: 1.01, transition: { duration: 0.25 } }}
          className="group relative rounded-3xl p-7 bg-white/80 backdrop-blur-xl border-2 border-[#E8DEC8] hover:border-amber-500 shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden flex flex-col justify-between"
        >
          ...
          <div className="relative z-10 space-y-5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-heading text-xs uppercase font-extrabold tracking-widest text-amber-700">
              ...
              <span className="px-3 py-1 rounded-full bg-amber-50 text-amber-800 font-heading text-xs font-bold uppercase tracking-wider">
                Peak Window
              </span>
            </div>
            ...
            <div className="space-y-4 pt-1">
              <div className="p-4 rounded-2xl bg-gradient-to-r from-amber-50 to-orange-50/80 border border-amber-200/90 shadow-2xs space-y-1">
                ...
                <p className="font-heading text-lg sm:text-xl font-bold text-[#12213B]">
                  {data.bestTimeToVisit?.idealMonths || 'October to March (Optimal Season)'}
                </p>
                <p className="font-sans text-sm text-neutral-700 leading-relaxed pt-1">
              ...
              {data.bestTimeToVisit?.crowdPacing && (
                <div className="p-3.5 rounded-2xl bg-[#FAF7F2]/90 border border-[#E8DEC8] flex items-start gap-3">
                  ...
                  <p className="font-sans text-sm text-neutral-700 leading-relaxed font-medium">
                    {data.bestTimeToVisit.crowdPacing}
                  </p>
                </div>
              )}
              ...
              {(data.bestTimeToVisit?.advisoryNote || data.bestTimeToVisit?.advisory) && (
                <div className="p-3.5 rounded-2xl bg-orange-50/90 border border-orange-200 flex items-start gap-2.5 text-orange-950 font-sans text-xs sm:text-sm">
        ...
        {/* CARD 4: NEARBY HORIZONS & EXPEDITIONS */}
        <motion.div
          variants={cardItemVariants}
          whileHover={{ y: -5, scale: 1.01, transition: { duration: 0.25 } }}
          className="group relative rounded-3xl p-7 bg-white/80 backdrop-blur-xl border-2 border-[#E8DEC8] hover:border-emerald-600 shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden flex flex-col justify-between"
        >
          ...
          <div className="relative z-10 space-y-5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-heading text-xs uppercase font-extrabold tracking-widest text-emerald-700">
                    Perimeter Horizons
                  </span>
                  <h4 className="font-heading text-2xl font-extrabold text-[#12213B] tracking-tight">
              ...
              <span className="px-3 py-1 rounded-full bg-emerald-50 text-emerald-800 font-heading text-xs font-bold uppercase tracking-wider">
                Curated Radii
              </span>
            </div>
            ...
            <div className="space-y-3 pt-1">
              {(data.nearbyPlaces || []).map((place, idx) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <h5 className="font-heading font-bold text-sm sm:text-base text-[#12213B] group-hover/row:text-emerald-700 transition-colors">
                        {place.name}
                      </h5>
                      <span className="font-sans text-xs text-neutral-500 font-medium">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="font-heading text-xs font-bold uppercase tracking-wider bg-white border border-[#E8DEC8] text-neutral-700 px-3 py-1 rounded-xl shadow-2xs">
                        {place.tag}
                      </span>
