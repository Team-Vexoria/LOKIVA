...
/**
 * Maps any category string to one of the canonical image pool keys
 */
...
/**
 * Returns a guaranteed unique image for an experience, ensuring it is never shared
 * with any other item in the current view.
 */
export function getUniqueImageForExperience(
  ...
  // 3. Fallback to category pool with anti-collision search
  ...
  // 4. Exhausted category pool: search across all pools to guarantee uniqueness
